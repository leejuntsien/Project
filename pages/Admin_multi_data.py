...
st.set_page_config(page_title="Multi-Patient Data Comparison", layout="wide")
...
def get_all_patients():
    """Get all patients"""
    import psycopg2
    from backend_patient_info import get_db_connection
...
# Title
st.title("Multi-Patient Data Comparison")

# Get all patients for selection
all_patients = get_all_patients()
patient_options = {p[0]: f"{p[1]} (ID: {p[0]})" for p in all_patients}

# Initialize session state for selected patients if not exists
if 'selected_patients' not in st.session_state:
    # Default to first patient if available
    st.session_state.selected_patients = [list(patient_options.keys())[0]] if patient_options else []
...
if not selected_patients:
    st.warning("Please select at least one patient to analyze")
    if st.button("← Back to Patient List"):
        st.switch_page("pages/_admin_patient_info.py")
    st.stop()

# Patient overlay option (only show if multiple patients selected)
overlay_patients = False
if len(selected_patients) > 1:
    overlay_patients = st.checkbox("Overlay patient data in visualizations", value=False, 
                                  help="If checked, data from all patients will be shown in the same plots")

# Get data IDs for all selected patients
all_data_ids = {}
for patient_id in selected_patients:
    data_ids = get_patient_data_ids(patient_id)
    if data_ids:
        # Format data IDs for display with patient info
        for d_id, created_at in data_ids:
            ...
            all_data_ids[d_id] = {
                'display': f"{patient_options[patient_id]} - Data ID: {d_id} (Created: {timestamp})",
                'patient_id': patient_id,
                'created_at': timestamp
            }

if not all_data_ids:
    st.warning(f"No data found for the selected patients")
    if st.button("← Back to Patient List"):
        st.switch_page("pages/_admin_patient_info.py")
    st.stop()
...
if 'selected_data_ids' not in st.session_state or not set(st.session_state.selected_data_ids).issubset(set(all_data_ids.keys())):
    # Default to selecting one data ID per patient if available
    default_selection = []
    patient_added = set()
    for d_id, info in all_data_ids.items():
        if info['patient_id'] not in patient_added:
            default_selection.append(d_id)
            patient_added.add(info['patient_id'])
...
overlay_data_ids = False
if len(selected_data_ids) > 1:
    overlay_data_ids = st.checkbox("Overlay data IDs in visualizations", value=True, 
...
if col4.button ("More analysis tools?"):
    st.switch_page("pages/admin_advanced_anal.py")
